...
#include "../../Sequences/PriorityQueue.hpp" // Приоритетная очередь для Дийкстры
#include "../../Sequences/Queue.hpp" // Обычная очередь для BFS
...
public:
    ...
    static DynamicArray<Edge<TVertex, TWeight>> FindShortestPath(const DirectedGraph<TVertex, TWeight>& graph, TVertex& source, TVertex& dest){
        ...
        DynamicArray<Edge<TVertex, TWeight>> shortestPath(path.GetLength());
        for (int i = path.GetLength() - 1, j = 0; i >= 0; i--, j++) {
